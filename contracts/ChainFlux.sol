...
contract ChainFlux is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC20Upgradeable {
    ...
    enum Market { GAS, LIQUIDATIONS, TXS_PER_BLOCK, RESERVED_3, RESERVED_4, RESERVED_5, RESERVED_6, RESERVED_7 }
    ...
    event PositionOpened(uint256 indexed id, address indexed trader, Market market, Direction direction, uint256 collateral, uint8 leverage, uint256 liquidationPrice, uint256 cftMinted);
    ...
    modifier validLeverage(uint8 leverage) {
        require(leverage >= 2 && leverage <= 30, "bad leverage");
        require(leverage <= getMaxLeverage(msg.sender), "tier too low");
        _;
    }
    ...
    function initialize(address _feeRecipient, address _keeper) external initializer {
    ...
    function initializeV3(address founderWallet) external onlyOwner {
        require(!v3Initialized, "done");
        require(founderWallet != address(0), "zero addr");
        v3Initialized = true;
        uint256 r = 700_000_000 * 1e18;
        _mint(address(this), r);
        cftRewardsPool = r;
        _mint(founderWallet, 300_000_000 * 1e18);
    }

    function topUpRewardsPool(uint256 amount) external {
        require(amount > 0, "zero");
        _transfer(msg.sender, address(this), amount);
        cftRewardsPool += amount;
    }

    function getTier(address user) public view returns (uint8) {
        uint256 b = balanceOf(user);
        if (b >= TIER_DIAMOND) return 4;
        if (b >= TIER_GOLD)    return 3;
        if (b >= TIER_SILVER)  return 2;
        if (b >= TIER_BRONZE)  return 1;
        return 0;
    }

    function getMaxLeverage(address user) public view returns (uint8) {
        uint8 t = getTier(user);
        if (t == 4) return LEV_DIAMOND;
        if (t == 3) return LEV_GOLD;
        if (t == 2) return LEV_SILVER;
        if (t == 1) return LEV_BRONZE;
        return LEV_UNRANKED;
    }
    ...
    function openPosition(uint8 marketId, uint8 dirId, uint8 leverage) external payable nonReentrant validLeverage(leverage) {
        require(msg.value >= MIN_POSITION, "too small");
        Market market = Market(marketId);
        Direction direction = Direction(dirId);
        require(markets[market].price > 0, "no feed");
        require(block.timestamp - markets[market].updatedAt <= PRICE_STALENESS, "stale");
        _processOpen(market, direction, leverage);
    }

    function _processOpen(Market market, Direction direction, uint8 leverage) internal {
        uint256 fee        = (msg.value * FEE_BPS) / BPS_DENOM;
        uint256 collateral = msg.value - fee;
        totalFeesCollected += fee;
        (bool feeOk,) = feeRecipient.call{value: fee}("");
        require(feeOk, "fee fail");
        ...
        positions[id] = Position({
            trader: msg.sender, market: market, direction: direction,
            collateral: collateral, size: collateral * leverage,
            entryPrice: entryPrice, openedAt: block.timestamp,
            cftMinted: 0, open: true, leverage: leverage,
    ...
    function closePosition(uint256 id) external nonReentrant {
        ...
        require(pos.trader == msg.sender, "not yours");
        uint256 currentPrice = markets[pos.market].price;
        require(currentPrice > 0, "no feed");
        bool earnsCFT = block.timestamp - pos.openedAt >= CFT_MIN_HOLD;
        int256  pnl    = _calculatePnL(pos, currentPrice);
        uint256 payout = _resolvePayout(pos.collateral, pnl);
        uint256 cftReward = 0;
        if (earnsCFT) {
            cftReward = _computeCFTReward(pos.collateral, pos.leverage, pnl >= 0);
    ...
    function liquidate(address trader, uint8 marketId) external nonReentrant {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (p.open && uint8(p.market) == marketId) { posId = ids[i]; break; }
        }
        require(posId != type(uint256).max, "no position");
        Position storage pos = positions[posId];
        uint256 currentPrice = markets[pos.market].price;
        require(currentPrice > 0, "no feed");
        require(_isLiquidatable(pos, currentPrice), "not liqable");
        ...
        if (bonus > 0 && address(this).balance >= bonus) {
            ...
            require(ok, "bonus fail");
        }
        if (remaining > 0 && address(this).balance >= remaining) {
            (bool ok2,) = feeRecipient.call{value: remaining}("");
            require(ok2, "remain fail");
        }
        emit PositionLiquidated(posId, trader, msg.sender, bonus);
    }

    function _computeCFTReward(uint256 collateral, uint8 leverage, bool won) internal pure returns (uint256) {
        uint256 cft = collateral * uint256(leverage) * CFT_MULTIPLIER;
    ...
    function _computeLiqPrice(uint256 entryPrice, uint8 leverage, Direction direction) internal pure returns (uint256) {
        uint256 move = (LIQ_THRESHOLD_BPS * PRICE_PRECISION) / (BPS_DENOM * leverage);
        if (direction == Direction.LONG) {
            return entryPrice - (entryPrice * move / PRICE_PRECISION);
        } else {
            return entryPrice + (entryPrice * move / PRICE_PRECISION);
        }
    }
    ...
    function _resolvePayout(uint256 collateral, int256 pnl) internal pure returns (uint256) {
        if (pnl >= 0) return collateral + uint256(pnl);
        uint256 loss = uint256(-pnl);
        return loss >= collateral ? 0 : collateral - loss;
    }

    function _closePosition(uint256 id, Position storage pos, uint256 payout, int256 pnl, uint256 cftRewarded) internal {
        if (pos.direction == Direction.LONG) {
            markets[pos.market].longOI  -= pos.collateral;
        } else {
            markets[pos.market].shortOI -= pos.collateral;
        ...
        pos.open = false;
        if (payout > 0) {
            require(address(this).balance >= payout, "low bal");
    ...
    function getAllMarkets() external view returns (MarketState memory gas, MarketState memory liquidations, MarketState memory txsPerBlock) {
        return (markets[Market.GAS], markets[Market.LIQUIDATIONS], markets[Market.TXS_PER_BLOCK]);
    }

    function getTierInfo(address user) external view returns (uint8 tier, uint256 cftBalance, uint8 maxLeverage, uint256 nextTierThreshold) {
        tier        = getTier(user);
    ...
    function setFeeRecipient(address _newRecipient) external onlyOwner {
        require(_newRecipient != address(0), "zero addr");
